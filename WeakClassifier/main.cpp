...
vector<int> get_3x1_wavelets(vector<int> &mask)
{
    ...
    for (int i = 2; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            ret.push_back(mask[(i - 1) * SIZE + j] - mask[(i - 2) * SIZE + j] - mask[i * SIZE + j]);
        }
    }
...
vector<int> get_2x2_wavelets(vector<int> &mask)
{
    ...
    for (int i = 1; i < SIZE; i++) {
        for (int j = 1; j < SIZE; j++) {
            ret.push_back(mask[i * SIZE + (j - 1)] + mask[(i - 1)* SIZE + j] - mask[(i - 1) * SIZE + (j - 1)] - mask[i * SIZE + j]);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            ...
            if (i <= 1 && j <= 1)
                printf("(%d, %d) = %d -> ", i, j, resized_img.at<uchar>(i, j));
